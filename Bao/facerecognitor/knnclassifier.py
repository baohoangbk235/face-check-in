...
class KNNClassifier(object):
            ...
            for class_dir in os.listdir(train_dir):
                ...
                for img_path in class_images[-50:]:
                    ...
                    if len(class_images) <= 1:
                        ...
                        #     face_bounding_boxes.append([top,left+width,top+height,left])
                        # face_bounding_boxes_flip = face_recognition.face_locations(image_flip)
                    else:
                        h, w = image.shape[:2]
                        face_bounding_boxes = [[0, w, h, 0]]
                        # face_bounding_boxes_flip = [[0,w,h,0]]
                    ...
                    if len(face_bounding_boxes) != 1:
                        ...
                        y.append(class_dir)
                        # X.append(face_recognition.face_encodings(image_flip, known_face_locations=face_bounding_boxes_flip)[0])
                        # y.append(class_dir)
    ...
    def predict_image(self, X_img, distance_threshold=0.6, scale=1.0, cropped=False):
        if not cropped:
            if scale < 1.0:
